...
describe('Redirects', () => {
  describe('meet.serlo.org', () => {
    test('meet.serlo.org', async () => {
      redirects1('meet.serlo.org', 'https://meet.google.com/vtk-ncrc-rdp', 302)
    })
    // {
    ...
    test('meet.serlo.org/einbindung', async () => {
      redirects1(
        'meet.serlo.org/einbindung',
        'https://meet.google.com/qzv-ojgk-xqw',
        302
      )
    })
    ...
    test('meet.serlo.org/begleitung', async () => {
      redirects1(
        'meet.serlo.org/begleitung',
        'https://meet.google.com/kon-wdmt-yhb',
        302
      )
    })
    ...
    test('meet.serlo.org/labschool', async () => {
      redirects1(
        'meet.serlo.org/labschool',
        'https://meet.google.com/cvd-pame-zod',
        302
      )
    })
    ...
    test('returns 404 when meet room is not defined', async () => {
      redirects7('https://meet.serlo.local/def')
    })
    // test('returns 404 when meet room is not defined', async () => {
  ...
  test.each(['/labschool', '/labschool/'])('serlo.org%s', async (path) => {
    redirects1(
      `https://de.serlo.local${path}`,
      'https://labschool.serlo.local/',
      301
    )
  })
  ...
  test('serlo.org/*', async () => {
    redirects1('https://serlo.local/foo', 'https://de.serlo.local/foo', 302)
  })
  // test('serlo.org/*', async () => {
  ...
  // })
  test('serlo.org/*', async () => {
    redirects1('https://www.serlo.local/foo', 'https://de.serlo.local/foo', 302)
  })
  ...
  describe('redirects to current path of an resource', () => {
    ...
    test('redirects when current path is different than target path', async () => {
      redirects1(
        'https://en.serlo.org/sexed',
        'https://en.serlo.org/sex-ed',
        301
      )
    })
    // test('redirects when current path is different than target path', async () => {
    ...
    test('no redirect when current path is different than given path and XMLHttpRequest', async () => {
      redirects4('https://en.serlo.org/sexed', 'https://en.serlo.org/sexed')
    })
    ...
    test('no redirect when current path is the same as given path', async () => {
      redirects3('https://en.serlo.org/sex-ed', 'https://en.serlo.org/sex-ed')
    })
    ...
    test('no redirect when requested entity has no alias', async () => {
      redirects2(
        { id: 128620, __typename: 'ArticleRevision' },
        'https://de.serlo.org/128620',
        'https://de.serlo.org/128620'
      )
    })
    ...
    test('redirects to alias of course when list of course pages is empty', async () => {
      redirects6(
        { id: 42, __typename: 'Course', alias: '/course', pages: [] },
        'https://en.serlo.org/42',
        'https://en.serlo.org/course',
        301
      )
    ...
    test('no redirect when current path cannot be requested', async () => {
      redirects5('https://en.serlo.org/path', 'https://en.serlo.org/path')
    })
